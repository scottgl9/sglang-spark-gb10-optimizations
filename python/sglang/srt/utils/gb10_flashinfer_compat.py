...
def _copy_fp4_header(data_dir: str) -> None:
    ...
    # Look for the header in sglang.kernels.aot (formerly sgl-kernel/csrc)
    import sglang

    sglang_root = os.path.dirname(os.path.abspath(sglang.__file__))
    src = os.path.join(
        sglang_root, "kernels", "aot", "csrc", "nv_fp4_dummy.h"
    )
    if not os.path.exists(src):
        # Try relative to this file
        src = os.path.join(
            os.path.dirname(__file__), "..", "..",
            "kernels", "aot", "csrc", "nv_fp4_dummy.h"
        )
